...
# Main function
chirps2rgb <- function(date = dts[1]){
  # CHIRPS base URL
  ...
  if(!file.exists(ifile)){
    ...
    img <- imager::load.image(aux) %>% imager::resize(512,512); rm(aux)
    # Assigning raster values to gray image format
    img[,,1,1] <- raster::as.matrix(r_nrm)
    img[,,1,2] <- raster::as.matrix(r_nrm)
    img[,,1,3] <- raster::as.matrix(r_nrm)
